...
from ml.utils import get_single_tag_keys, get_choice, is_skipped
...
class ImageClassifierAPI(LabelStudioMLBase):
    ...
    def predict(self, tasks, **kwargs):
        ...
        for x in range(len(image_urls)):
            print(image_urls[x].split('data')[1])
            image_urls[x] = "http://localhost:8080/static/random_choice_project"+image_urls[x].split('data')[1]
            print(image_urls[x])
